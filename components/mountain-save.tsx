...
export function SavedCollections(){
  ...
  if(!savedPlaces.length&&!savedMountains.length)return <div className="empty"><Heart size={34}/><h2>아직 저장한 장소나 코스가 없어요</h2><p>가고 싶은 장소와 산행 코스를 저장해 보세요.</p><Link href="/blog/seoul-mountains" className="button">서울 산 비교하기 <ArrowRight size={17}/></Link></div>;
  return <>{savedPlaces.length>0&&<section><h2>저장한 장소</h2><div className="saved-simple-grid">{savedPlaces.map(x=><Link key={x.id} href={'/places/'+x.id}><strong>{x.name}</strong><span>{x.kind}</span></Link>)}</div></section>}{savedMountains.length>0&&<section className="saved-mountains"><h2>저장한 산행 코스</h2><div className="mountain-grid">{savedMountains.map(x=><article className="mountain-card compact-mountain" key={x.slug}><Link href={'/blog/seoul-mountains/'+x.slug}><Image src={x.images[0].src} alt={x.images[0].alt} width={600} height={450}/></Link><div><span>{x.difficulty} · {x.routeType}</span><h2><Link href={'/blog/seoul-mountains/'+x.slug}>{x.displayName||x.name}</Link></h2><p>{x.summary}</p></div></article>)}</div></section>}</>;
}
