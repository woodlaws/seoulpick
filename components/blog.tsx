...
export function BlogListing({categorySlug,query,page}:{categorySlug?:string;query:string;page:number}){
  ...
  return <><nav className="blog-filters" aria-label="블로그 분류"><Link className={!category?'active':''} href="/blog">전체</Link>{blogCategories.map(c=><Link key={c.slug} className={category?.slug===c.slug?'active':''} href={'/blog/category/'+c.slug}>{c.label}</Link>)}<Link href="/mountains">서울 산 BEST 10</Link></nav>
    {!category&&!normalized&&<section className="mountain-feature"><Image src="/mountains/inwangsan/01.jpg" alt="인왕산과 서울 도심 전망" width={700} height={470} sizes="(max-width: 700px) 100vw, 240px"/><div><span className="accent-caption">서울픽 추천 10선</span><h2>서울 산 BEST 10</h2><p>지하철로 떠나는 서울의 산과 전망 산책길. 난이도와 시간을 비교해 내게 맞는 산을 골라보세요.</p><Link href="/mountains" className="text-link">10개 산 비교하기 <ArrowRight size={16}/></Link></div></section>}
